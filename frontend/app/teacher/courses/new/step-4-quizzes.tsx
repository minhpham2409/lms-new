    ...
    <div className="space-y-6 animate-scale-in">
      ...
      {active && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl rounded-xl bg-[var(--background)] border border-[var(--border)] p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4 mb-5">
              ...
              <button type="button" onClick={() => setActiveLesson(null)} className="btn-ghost px-2 py-2"><X className="w-4 h-4" /></button>
            </div>

            <div className="flex gap-2 mb-4">
              <button type="button" onClick={() => setAiMode("text")} className={aiMode === "text" ? "btn-primary text-sm" : "btn-secondary text-sm"}>Dán text</button>
              <button type="button" onClick={() => setAiMode("file")} className={aiMode === "file" ? "btn-primary text-sm" : "btn-secondary text-sm"}><Upload className="w-4 h-4" /> Upload file</button>
            </div>

            <div className="grid gap-3 sm:grid-cols-[120px_1fr] mb-4">
              <input type="number" min={1} max={30} value={aiCount} onChange={(e) => setAiCount(Math.max(1, Math.min(30, Number(e.target.value) || 1)))} className="input-base" />
              <div className="flex gap-2 flex-wrap">
                {difficulties.map((item) => (
                  <button key={item.value} type="button" onClick={() => setAiDifficulty(item.value)} className={aiDifficulty === item.value ? "btn-primary text-xs" : "btn-secondary text-xs"}>
                    {item.label}
                  </button>
                ))}
              </div>
            ...
            {aiMode === "text" ? (
              <textarea value={aiText} onChange={(e) => setAiText(e.target.value)} className="input-base w-full min-h-[220px] mb-4" placeholder="Dán nội dung bài học để Gemini tạo câu hỏi..." />
            ) : (
              <button type="button" onClick={() => fileRef.current?.click()} className="w-full min-h-[180px] mb-4 border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2" style={{ borderColor: "var(--border)" }}>
                <input ref={fileRef} type="file" accept=".pdf,.docx,.pptx,.xlsx,.txt,.csv" className="hidden" onChange={(e) => setAiFile(e.target.files?.[0] || null)} />
            ...
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setActiveLesson(null)} className="btn-secondary">Hủy</button>
