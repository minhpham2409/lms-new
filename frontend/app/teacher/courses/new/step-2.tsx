    ...
    <div className="card-base animate-scale-in">
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-bold flex items-center gap-2 text-lg">
          <Layers className="w-5 h-5" style={{ color: "#94A3B8" }} /> Bước 2: Thêm chương (Sections)
        </h3>
        ...
        <div className="space-y-4">
          {sections.map((sec: any, index: number) => (
            <div key={sec.id} className="p-4 rounded-xl flex items-center gap-3 transition-all" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
              <div className="w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm" style={{ background: "rgba(148,163,184,0.15)", color: "#94A3B8" }}>
                {index + 1}
              </div>
