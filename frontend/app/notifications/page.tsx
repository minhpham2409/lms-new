...
import { Bell, CheckCircle2, Clock, Info, Loader2 } from "lucide-react";
                ...
                return (
                  <div key={n.id}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs mt-1" style={{ color: "var(--foreground-muted)" }}>{displayMessage}</p>
                      <div className="flex items-center gap-3 mt-2">
                        <p className="text-xs" style={{ color: "var(--foreground-muted)" }}>
                          {new Date(n.createdAt).toLocaleDateString("vi-VN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                        </p>
                        {notifLink && (
                          <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: "rgba(124,58,237,0.15)", color: "#a78bfa" }}>
                            Xem chi tiết →
                          </span>
                        )}
                      </div>
